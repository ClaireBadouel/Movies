...
def init_db(
    ...
    DB_PATH=DB_PATH,
    CSV_NAME=CSV_NAME,
    COLUMNS_DATABASE=COLUMNS_DATABASE_SQLITE,
    COLUMNS_CSV_type=COLUMNS_CSV_type,
    ...
    CSV_DATA = pd.read_csv(os.path.join(DB_PATH, CSV_NAME))[COLUMNS_CSV.keys()].rename(
        columns=COLUMNS_CSV
    )
    print(os.path.join(DB_PATH, CSV_NAME), " is loaded")

    # Modify the genres column to be a list of the genres for each row
    try:
        CSV_DATA["genres"] = CSV_DATA["genres"].apply(
            lambda x: ", ".join([x_["name"] for x_ in ast.literal_eval(x)])
        )
    ...
    for col in COLUMNS_CSV_type.keys():
        if COLUMNS_CSV_type[col] != "str":
            CSV_DATA[col] = pd.to_numeric(
                CSV_DATA[col], downcast=COLUMNS_CSV_type[col], errors="coerce"
            )
            print(
                col,
                " type is set to ",
    ...
    c.execute(
        ...
        + ", ".join([k + " " + COLUMNS_DATABASE[k] for k in COLUMNS_DATABASE.keys()])
        + ")"
    )

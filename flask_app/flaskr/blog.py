...
#http DELETE localhost:5000/movies/movie_id
...
def show_OR_delete_OR_update_movie(movie_id):
    ...
    if request.method == 'PUT':
        with get_db() as conn:
            ...
            if request.method == 'PUT':
                res = c.execute(
                    'SELECT EXISTS(SELECT * FROM movies WHERE id==?)',
                    (movie_id,)
                    ).fetchall()
                if res[0][0] != 1:
                    abort(404)
...
def api_filter():
    ...
    like_request = []
    if all([key in ['genre', 'before', 'after', 'vote_average'] for key in query_parameters.keys()]):
